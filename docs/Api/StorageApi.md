...
```php
storageBatchUpdate($table, $filter, $app_key, $request_body, $replace): \ZSGF\Client\ZSGF\Client\Model\BooleanApiResponse
```
...
$app_key = 'app_key_example'; // string
$request_body = NULL; // mixed[] | 更新的字段
$replace = false; // bool | 是否为全量替换，默认为 false

try {
    $result = $apiInstance->storageBatchUpdate($table, $filter, $app_key, $request_body, $replace);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling StorageApi->storageBatchUpdate: ', $e->getMessage(), PHP_EOL;
}
```
...
| **filter** | **string**| 查询条件 | |
| **app_key** | **string**|  | |
| **request_body** | [**mixed[]**](../Model/mixed.md)| 更新的字段 | |
